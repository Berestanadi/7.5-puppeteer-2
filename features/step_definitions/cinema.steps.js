...
const { getText, days, movieTime, seats, clickElement } = require("../../lib/command");
...
When("user choose movie {string} and movieTime {string}", {timeout: 60000}, async function (string, string2) {
  return await movieTime(this.page, string, string2);
});

When("user choose seats row {string} and seat {string}", {timeout: 60000}, async function (string, string2) {
